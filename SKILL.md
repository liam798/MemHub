...
- **Review 场景**：在整体 Review 报告中追加一个小节，推荐标题为「按 MemHub 知识库规则需整改：」。该小节只列出**依据知识库规则判断需要整改**的条目，不必罗列已符合或与本次任务无关的规则。每条按以下格式说明：
...
- **示例**：
  ...
  ```
- **无整改项**：可写「按 MemHub 知识库规则需整改：无」（表示本次检查已满足相关规则），或省略该章节。
